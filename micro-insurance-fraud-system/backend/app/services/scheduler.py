...
async def run_fraud_check_job():
    ...
    try:
        ...
        # Query all recent transactions joined with their features
        ...
        for tx, feat in records:
            ...
            # Also check pre-scored feature risk_score
            feat_score = float(feat.risk_score) if feat.risk_score is not None else 0.0
            feat_score_pct = feat_score * 100.0 if feat_score <= 1.0 else feat_score
            final_score_pct = max(score_pct, feat_score_pct)
            ...
            if final_score_pct >= 70.0:
                ...
                if not pending_alert:
                    ...
                    flag_reason_str = str(scored.get("flag_reason", f"Risk score {final_score_pct:.1f}% flagged by ML model"))
                    logger.info(f"High risk transaction detected! Saving PENDING alert to DB: Tx ID {tx.id}, Agent {tx.agent_id}, Score {final_score_pct:.1f}%")
                    
                    # Persist alert to database
                    alert_obj = FraudAlert(
                        agent_id=tx.agent_id,
                        transaction_id=tx.id,
                        risk_score=final_score_pct,
